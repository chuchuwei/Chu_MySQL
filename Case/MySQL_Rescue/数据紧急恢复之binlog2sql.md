...
# --start-datetime 起始解析时间，格式'%Y-%m-%d %H:%M:%S'。可选。默认不过滤。例：'2018-11-21 14:50:00'
...
#如果知道误删时的语句为delete、update 时，可将 --only-dml和 --sql-type结合使用
#使用binlog2sql必须选择二进制日志--start-file二进制日志，即在某一个二进制中寻找，适合能定位到哪个二进制日志文件的
```
...
```
shell> python binlog2sql.py -h127.0.0.1 -P3306 -uadmin -p'admin' -d test -t test3 test4 --start-file='mysql-bin.000002'
...
闪回详细介绍可参见example目录下《闪回原理与实战》[example/mysql-flashback-priciple-and-practice.md](https://github.com/danfengcao/binlog2sql/blob/master/example/mysql-flashback-priciple-and-practice.md)
...
```shell
数据误操作 update
root@MySQL-01 13:58:  [test1]> select * from test123;
+------+----------+
| id   | name     |
+------+----------+
|    5 | xiaohong |
...
root@MySQL-01 13:58:  [test1]> update test123 set name='tomq' where id=1;
Query OK, 1 row affected (0.02 sec)
Rows matched: 1  Changed: 1  Warnings: 0

1 # 解析：
python binlog2sql.py -utest -pwaqb1314 --start-file='mybinlog.000033' --start-datetime='2018-11-21 13:50:00' --stop-datetime='2018-11-21 14:05:00' -dtest1 -ttest123 --only-dml

INSERT INTO `test1`.`test123`(`id`, `name`) VALUES (5, 'xiaohong'); #start 6124 end 6295 time 2018-11-21 13:58:31
INSERT INTO `test1`.`test123`(`id`, `name`) VALUES (4, 'jack'); #start 6391 end 6558 time 2018-11-21 13:58:31
INSERT INTO `test1`.`test123`(`id`, `name`) VALUES (3, 'alice'); #start 6654 end 6822 time 2018-11-21 13:58:32
INSERT INTO `test1`.`test123`(`id`, `name`) VALUES (2, 'xiaoming'); #start 6918 end 7089 time 2018-11-21 13:58:32
INSERT INTO `test1`.`test123`(`id`, `name`) VALUES (1, 'tom'); #start 7185 end 7351 time 2018-11-21 13:58:32
UPDATE `test1`.`test123` SET `id`=1, `name`='tomq' WHERE `id`=1 AND `name`='tom' LIMIT 1; #start 7447 end 7629 time 2018-11-21 14:01:08

2 # 解析完以后可在配合pos点准确定位
生成回滚语句
python binlog2sql.py -utest -pwaqb1314 --start-file='mybinlog.000033' --start-datetime='2018-11-21 13:50:00' --stop-datetime='2018-11-21 14:05:00' -dtest1 -ttest123 --only-dml -B > /tmp/rollback.sql

UPDATE `test1`.`test123` SET `id`=1, `name`='tom' WHERE `id`=1 AND `name`='tomq' LIMIT 1; #start 7447 end 7629 time 2018-11-21 14:01:08
DELETE FROM `test1`.`test123` WHERE `id`=1 AND `name`='tom' LIMIT 1; #start 7185 end 7351 time 2018-11-21 13:58:32
DELETE FROM `test1`.`test123` WHERE `id`=2 AND `name`='xiaoming' LIMIT 1; #start 6918 end 7089 time 2018-11-21 13:58:32
DELETE FROM `test1`.`test123` WHERE `id`=3 AND `name`='alice' LIMIT 1; #start 6654 end 6822 time 2018-11-21 13:58:32
DELETE FROM `test1`.`test123` WHERE `id`=4 AND `name`='jack' LIMIT 1; #start 6391 end 6558 time 2018-11-21 13:58:31
DELETE FROM `test1`.`test123` WHERE `id`=5 AND `name`='xiaohong' LIMIT 1; #start 6124 end 6295 time 2018-11-21 13:58:31

3 # 导入：
mysql -u -p < /tmp/rollback.sql
```
